...
class Experimental(EncoderOption):
    name = 'experimental'
...
class Options(object):
    ...
    def get_option_by_name(self, option_name):
        for opt in self.options:
            if opt.__class__.__name__.lower() == option_name.lower():
                return opt
    ...
    def incompatible_options(self, other):
        ...
        incompatible_options = Options(False)
    ...
    def __copy__(self):
        new = type(self)(unique=self.unique)
        for opt in self.options:
            new.add_option(copy(opt))
        return new

    def __hash__(self):
        opts = sorted(self.options, key=lambda opt: opt.__class__.__name__ + str(opt.value))
        options = ''.join(list(map(str, opts)))
        return hash(options)
